...
class GloVeModel():
    ...
    def fit_to_corpus(self, corpus):
        self.__fit_to_corpus(corpus, self.max_vocab_size, self.min_occurrences,
                             self.left_context, self.right_context, self.max_occurrences)
        self.__build_graph()
    ...
    def __fit_to_corpus(self, corpus, vocab_size, min_occurrences, left_size, right_size, 
        ...
        if os.path.isfile('words.pk'):
            self.__words = pickle.load(open( 'words.pk', "rb" ))
            self.__word_to_id = pickle.load(open( 'word_id.pk', "rb" ))
            self.__cooccurrence_matrix = pickle.load(open( 'cooccurrence.pk', "rb" ))
        else:
            ...
            with gfile.FastGFile('words.pk', 'wb') as f:
                pickle.dump(self.__words, f);
            with gfile.FastGFile('word_id.pk', 'wb') as f:
                pickle.dump(self.__word_to_id, f);
            with gfile.FastGFile('cooccurrence.pk', 'wb') as f:
        ...
        with tf.device('/gpu:0'):
            with tf.Session(graph=self.__graph, config=config) as session:
                ...
                if should_write_summaries:
                    ...
                    with gfile.FastGFile('graph.pg', 'wb') as f:
    ...
    def write_metadata(self):
        with open("logs/metadata.tsv", "wb") as myfile:
...
def _plot_with_labels(low_dim_embs, labels, path, size, final):
    ...
    for i, label in enumerate(labels):
        ...
        with open("logs/metadata.tsv", "a") as myfile:
            myfile.write(label + '\n')
    if path is not None:
        figure.savefig(path)
        plt.close(figure)